...
const defaultJsonPath = resolve(
  projectRoot,
  args.lifecycleOnly
    ? "test-results/react-panel-lifecycle-performance.json"
    : "test-results/react-panel-performance.json"
);
const lifecycleConfiguration = resolveLifecycleConfiguration(args);

if (args.help) {
  console.log(`Usage: node scripts/measure-react-panel.mjs [--json PATH] [--evaluate PATH] [--lifecycle-only]

Creates repeatable Slice 1 React panel evidence:
  ...
  - legacy/React unpacked bytes, stored ZIP bytes, every JavaScript raw/gzip size, and isolation

The default JSON output is test-results/react-panel-performance.json (or
test-results/react-panel-lifecycle-performance.json in lifecycle-only mode).
...
if (args.inspectHarness) {
  const inspectionRoot = await mkdtemp(join(tmpdir(), "lsew-react-panel-inspect-"));
...
const temporaryRoot = await mkdtemp(join(tmpdir(), "lsew-react-panel-measure-"));
let server;
let browser;
...
try {
  ...
  if (args.lifecycleOnly) {
    ...
    await context.close();
    report.artifacts = {
      legacy: await measureArtifact("dist", join(temporaryRoot, "package-legacy")),
      react: await measureArtifact("dist-react", join(temporaryRoot, "package-react"))
    };
...
globalThis.__LSEW_REACT_PERF__ = { mount, ingest, cycle, dispose };
if (new URL(globalThis.location.href).searchParams.get("lifecycleOnly") !== "1") await mount(12);
document.documentElement.dataset.reactPerfReady = "true";
`;
}
...
async function measureHighVolume(context, url) {
  ...
  await cdp.send("Performance.enable");
  await page.goto(url, { waitUntil: "domcontentloaded" });
  await page.locator('html[data-react-perf-ready="true"]').waitFor();
  const before = await performanceMetrics(cdp);
  const result = await page.evaluate(() => globalThis.__LSEW_REACT_PERF__.ingest({ count: 180, intervalMs: 2 }));
  const after = await performanceMetrics(cdp);
  await page.close();
...
async function measureLifecycle(context, url, configuration) {
  ...
  for (let cycle = 0; cycle < configuration.warmupCycles; cycle += 1) {
    await page.evaluate((scenario) => globalThis.__LSEW_REACT_PERF__.cycle({ scenario, captureCount: 60 }), configuration.lifecycleScenario);
  ...
  for (let cycle = 1; cycle <= configuration.recordedCycles; cycle += 1) {
    const currentProbe = await page.evaluate(
      (scenario) => globalThis.__LSEW_REACT_PERF__.cycle({ scenario, captureCount: 60 }),
...
async function measureIsolation() {
  const legacy = await javascriptContents(resolve(projectRoot, "dist"));
  const react = await javascriptContents(resolve(projectRoot, "dist-react"));
  const panelPath = "extension/panel/index.js";
  const nonPanelDifferences = [...new Set([...legacy.keys(), ...react.keys()])]
    .filter((path) => path !== panelPath)
    .filter((path) => !legacy.get(path)?.equals(react.get(path) ?? Buffer.alloc(0)));
  const legacyPanel = legacy.get(panelPath)?.toString("utf8") ?? "";
  const reactPanel = react.get(panelPath)?.toString("utf8") ?? "";
  return {
    nonPanelJavaScriptIdentical: nonPanelDifferences.length === 0,
    nonPanelDifferences,
    legacyRendererOnly:
      legacyPanel.includes("LSEW_PANEL_RENDERER:legacy") &&
      !legacyPanel.includes("LSEW_PANEL_RENDERER:react"),
    reactRendererOnly:
      reactPanel.includes("LSEW_PANEL_RENDERER:react") &&
      !reactPanel.includes("LSEW_PANEL_RENDERER:legacy")
  };
}
...
function printSummary(report, jsonPath) {
  console.log("\nReact panel Slice 1 performance evidence");
  ...
  console.table([
    ...
    {
      metric: "Legacy / React stored ZIP",
      value: `${formatBytes(report.artifacts.legacy.storedZipBytes)} / ${formatBytes(report.artifacts.react.storedZipBytes)}`
    },
    {
      metric: "Legacy / React panel JS raw",
      value: `${formatBytes(report.artifacts.legacy.javascript["extension/panel/index.js"].rawBytes)} / ${formatBytes(report.artifacts.react.javascript["extension/panel/index.js"].rawBytes)}`
    }
  ]);
  console.log(`Environment: ${report.environment.chrome}; ${report.environment.platform} ${report.environment.architecture}; Node ${report.environment.node}`);
  console.log(`JSON: ${jsonPath}`);
