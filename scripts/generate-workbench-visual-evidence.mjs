...
function isReadabilitySetup(setup) {
...
function contactSheetScenarioIds(matrix) {
  return matrix
    .filter(({ id, production }) => id.startsWith("scenario-") || id.startsWith("readability-c-") || id.startsWith("local-injection-") || production.setup.startsWith("activity") || production.setup === "diagnostics" || isIntegratedDiagnosticSetup(production.setup) || isStorageHeadroomSetup(production.setup))
    .map(({ id }) => id);
}
...
async function captureProduction(runningBrowser, scenario, productionOverride = null) {
  ...
  try {
    ...
    if (scenario.production.setup.startsWith("scenario") || scenario.production.setup.startsWith("activity") || isLocalInjectionSetup(scenario.production.setup) || isReadabilitySetup(scenario.production.setup) || isIntegratedDiagnosticSetup(scenario.production.setup) || isStorageHeadroomSetup(scenario.production.setup) || ["more-actions-help", "clear-confirmation", "memory-operations", "diagnostics"].includes(scenario.production.setup)) {
    ...
    if (isReadabilitySetup(scenario.production.setup)) {
      const action = page.getByRole("navigation", { name: "Structural runtime scope" }).getByRole("treeitem").first();
      await action.scrollIntoViewIfNeeded();
      await action.focus();
      await page.keyboard.press("Tab");
      await page.keyboard.press("Shift+Tab");
...
async function prepareProductionState(page, setup, storageMode = "scenario") {
  ...
  if (isReadabilitySetup(setup)) {
    if (setup === "readability-scope") await page.getByRole("button", { name: "Scope", exact: true }).click();
    const scope = page.getByRole("navigation", { name: "Structural runtime scope" });
    await scope.waitFor({ state: "visible" });
    await scope.locator(".workbench-react__scope-identity", { hasText: "Inspected page" }).waitFor();
    if (setup === "readability") {
      await page.getByRole("region", { name: "Ordered Evidence" }).waitFor({ state: "visible" });
      await page.getByRole("grid", { name: "Ordered Lightstreamer Evidence" }).waitFor({ state: "visible" });
    }
    return;
  }
