...
for (const auth of ["off", "required"] as const) {
  for (const [name, width, height] of [["compact", 563, 700], ["normal", 900, 700], ["shallow", 900, 320], ["wide", 1440, 900]] as const) {
    for (const theme of ["dark", "light"] as const) {
      test(`Agent access is deliberate, reachable and revocable: ${name} ${theme} auth ${auth}`, async ({ page }, info) => {
        const errors: string[] = [];
        ...
        await expect(access.getByLabel("Require authentication")).not.toBeChecked();
        await expect(access).toContainText("Any local process can use this panel’s grant");
        if (auth === "required") {
          await access.locator("summary").filter({ hasText: "Connection options" }).click();
          await access.getByRole("checkbox", { name: "Require authentication" }).check();
          await page.screenshot({ path: info.outputPath(`agent-${name}-${theme}-options.png`) });
          await access.locator("summary").filter({ hasText: "Connection options" }).click();
        }
        ...
        await expect(access.getByRole("button", { name: "Disconnect agent" })).toBeFocused();
        ...
        if (auth === "required") await access.getByRole("button", { name: "Approve connection", exact: true }).click();
...
test("Cancelling a displayed comparison code leaves access off", async ({ page }) => {
  ...
  await page.locator("summary").filter({ hasText: "Connection options" }).click();
  await page.getByRole("checkbox", { name: "Require authentication" }).check();
  await page.getByRole("button", { name: "Connect agent" }).click();
...
test("Authentication mode remains visible after leaving and reopening a connected panel", async ({ page }) => {
  await page.goto("/?scenario=live-selected&agent=ready");
  await expect(page.locator("html")).toHaveAttribute("data-react-scene-ready", "true");
  await page.getByRole("button", { name: "More actions" }).click();
  await page.locator("summary").filter({ hasText: "Agent access" }).click();
  ...
  await expect(summary.locator("..")).toContainText("Authentication on");
  await expect(page.getByLabel("Require authentication")).toBeChecked();
});
