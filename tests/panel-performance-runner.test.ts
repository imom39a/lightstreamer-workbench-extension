...
const runner = join(rootDir, "scripts", "measure-panel.mjs");
...
describe("panel performance evidence runner", () => {
  it("exposes one repeatable build-and-measure command and documents its bounded evidence", () => {
    expect(packageJson.scripts["measure:panel"]).toBe(
      "npm run build && node scripts/measure-panel.mjs"
    );
  ...
  it("evaluates every lifecycle-only run while preserving the single-run lifecycle report seam", () => {
    const passing = writeReport({
      mode: "lifecycle-only",
...
function writeReport(report: unknown): string {
  const directory = mkdtempSync(join(tmpdir(), "lsew-panel-performance-test-"));
