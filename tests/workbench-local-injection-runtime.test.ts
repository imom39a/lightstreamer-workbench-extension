...
describe("WorkbenchRuntime Local Injection", () => {
  ...
  it.each([
    ["stale-target", { error: "target retired" }],
    ["listener-error", { attemptedCount: 2, deliveredCount: 1, failedCount: 1 }],
    ["listener-error", { attemptedCount: 1, deliveredCount: 0, failedCount: 1 }],
    ["wire-error", { error: "wire rejected" }],
    ["bridge-error", { error: "bridge unavailable" }],
    ["acknowledgement-unknown", { error: "acknowledgement lost" }]
  ] as const)("halts a Scenario after the coordinator settles %s without Local Evidence", async (status, facts) => {
    const executor = { execute: vi.fn(async () => result(status, facts)) };
    const runtime = createWorkbenchRuntime({ history: historyWithCommandTarget(), captureStatus: "capturing", localInjectionExecutor: executor });
    await flushAsync();
    ...
    await flushAsync();
    await flushAsync();
    expect(runtime.getSnapshot().scenario).toMatchObject({ phase: "stopped", run: { nextOrdinal: 1 } });
  ...
  it("does not carry a pre-Clear Server interleave into a later History interval", async () => {
    const history = historyWithCommandTarget();
    let execution = 0;
    const executor = { execute: vi.fn(async () => ++execution === 1
      ? result("wire-error", { error: "first Run stopped" })
      : result("success", { attemptedCount: 1, deliveredCount: 1, failedCount: 0 })) };
    const runtime = createWorkbenchRuntime({ history, captureStatus: "capturing", localInjectionExecutor: executor });
    await flushAsync();
    beginSelected(runtime);
    runtime.dispatch({ type: "convert-local-injection-to-scenario" });
    ...
    await flushAsync();
    await flushAsync();
    history.offer(commandEvent("pre-clear-server", "item-update", { update: { isSnapshot: false, command: "UPDATE", key: "order-1", fields: { command: "UPDATE", key: "order-1", qty: 8 }, changedFields: { qty: 8 } } }));
    await flushAsync();
    ...
    await history.offer(commandEvent("post-clear-boundary", "item-update", { update: { isSnapshot: false, command: "UPDATE", key: "order-1", fields: { command: "UPDATE", key: "order-1", qty: 9 }, changedFields: { qty: 9 } } })).settled;
    await flushAsync();
    runtime.dispatch({ type: "run-scenario-again" });
    expect(runtime.getSnapshot().scenario).toMatchObject({ phase: "review", run: { drifts: [] } });
    runtime.dispatch({ type: "step-next-scenario" });
    await flushAsync();
    await flushAsync();
    expect(runtime.getSnapshot().scenario).toMatchObject({ phase: "complete", run: { drifts: [] } });
    expect(executor.execute).toHaveBeenCalledTimes(2);
    runtime.dispose();
  });
  ...
  it("defers in-flight Server drift across the next Checkpoint and gates the following Injection", async () => {
    const history = historyWithCommandTarget();
    ...
    const executor = { execute: vi.fn(() => executor.execute.mock.calls.length === 1 ? pending : Promise.resolve(result("success", { attemptedCount: 1, deliveredCount: 1, failedCount: 0 }))) };
    const runtime = createWorkbenchRuntime({ history, captureStatus: "capturing", localInjectionExecutor: executor });
    await flushAsync();
    beginSelected(runtime);
    runtime.dispatch({ type: "convert-local-injection-to-scenario" });
    ...
    runtime.dispatch({ type: "update-scenario-checkpoint", checkpoint: {
      ...checkpoint,
      assertions: [{ id: "server-key", kind: "command-key-exists", item: { name: identity.itemName, position: 1 }, key: "server-key", expected: "present" }]
    } });
    runtime.dispatch({ type: "add-authored-scenario-step" });
    runtime.dispatch({ type: "review-scenario" });
    runtime.dispatch({ type: "step-next-scenario" });
    expect(runtime.getSnapshot().scenario?.runner?.phase).toBe("in-flight");

    const server = commandEvent("server-during-in-flight-9", "item-update", {
      update: { isSnapshot: false, command: "ADD", key: "server-key", fields: { command: "ADD", key: "server-key", qty: 9 }, changedFields: { command: "ADD", key: "server-key", qty: 9 } }
    });
    await history.offer(server).settled;
    resolveExecution(result("success", { attemptedCount: 1, deliveredCount: 1, failedCount: 0 }));
    await flushAsync();
    await flushAsync();
    expect(runtime.getSnapshot().scenario).toMatchObject({ phase: "paused", runner: { pauseReason: "USER" }, run: { nextMemberIndex: 1, drifts: [] } });

    runtime.dispatch({ type: "step-next-scenario" });
    expect(runtime.getSnapshot().scenario).toMatchObject({
      phase: "paused",
      run: { nextMemberIndex: 2, trace: [{ kind: "attempted" }, { kind: "checkpoint", status: "pass", assertions: [{ observed: { provenance: "server" } }] }] }
    });
    runtime.dispatch({ type: "step-next-scenario" });
    expect(runtime.getSnapshot().scenario).toMatchObject({ phase: "paused", runner: { pauseReason: "DRIFT_REVIEW_REQUIRED" }, run: { nextMemberIndex: 2, drifts: [{ evidence: { eventId: "server-during-in-flight-9" } }] } });
    expect(executor.execute).toHaveBeenCalledTimes(1);
    runtime.dispose();
  });
  ...
  it("freezes a fresh direct-execution preflight when the current listener set changes", async () => {
    ...
    beginSelected(runtime);
    runtime.dispatch({ type: "set-local-injection-json", text: updateDocument(7) });
    const draftText = runtime.getSnapshot().localInjection.draft?.rawText;
