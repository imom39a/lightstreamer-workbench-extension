...
describe("filter-impl-09 durable IndexedDB workload", () => {
  it("discovers all 3,842 COMMAND keys across 10,000 Evidence without payload hydration", async () => {
    ...
    try {
      const receipts = [];
      for (let sequence = 0; sequence < evidenceCount; sequence += 1) {
        receipts.push(durable.offer(workloadEvent(sequence)).settled);
      }
      await Promise.all(receipts);

      expect(durable.status().retained).toBe(evidenceCount);
      ...
      expect(initial).toMatchObject({ state: "AVAILABLE", distinctTotal: keyCount, baseEvidenceCount: evidenceCount });
      expect(first.value.telemetry).toMatchObject({ payloadHydrations: 0, lookupPayloadHydrations: 0, fullRetainedScan: true });
      ...
      expect(first.value.telemetry?.postingCandidates).toBe(evidenceCount);
      ...
      while (cursor !== null) {
        ...
        expect(next.value.telemetry).toMatchObject({ payloadHydrations: 0, lookupPayloadHydrations: 0, fullRetainedScan: true });
      ...
      for (let index = 0; index < keyCount; index += 1) {
        ...
        expect(seen.get(identity)).toBe(index < evidenceCount % keyCount ? 3 : 2);
      }
    } finally {
      await durable.close();
    }
