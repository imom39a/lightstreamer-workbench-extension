...
export type WorkbenchScenario = Readonly<{
  ...
  localInjection?: Readonly<{
    ...
    executorOutcome?:
      "pending" | "delivered" | "delayed" | "failed" | "partial" | "unknown";
...
export function isWorkbenchScenarioId(
  value: string,
): value is WorkbenchScenarioId {
  ...
  switch (id) {
    ...
    case "selected-local-evidence":
      return {
        id,
        initialEvents: canonical,
        selectedEventId: "scenario-event-5",
        captureStatus: "capturing",
      };
    ...
    case "activity-10k":
      return {
        ...
        initialEvents: highVolumeEvents(1, 10_000).map((event, index) => ({
          ...
          listener: {
            ...(event.listener ?? { id: `activity-listener-${index % 4}` }),
            metricOwner: true,
          },
        })),
    ...
    case "activity-graphical":
      return {
        ...
        initialEvents: highVolumeEvents(1, 40).map((event, index) => ({
          ...
          client: {
            ...(event.client ?? { id: "activity-graphical-client" }),
            id: event.client?.id ?? "activity-graphical-client",
            requestedMaxBandwidth: 10,
            realMaxBandwidth: index < 20 ? 5 : 7,
          },
          subscription: {
            ...event.subscription,
            id: `activity-graphical-subscription-${index % 12}`,
            requestedMaxFrequency: 4,
            realMaxFrequency: index < 20 ? 2 : 3,
          },
          ...(index === 0
            ? { source: "synthetic" as const, synthetic: true }
            : {}),
          listener:
            index % 3 === 0
              ? undefined
              : {
                  ...(event.listener ?? {
                    id: `activity-graphical-listener-${index % 4}`,
                  }),
                  metricOwner: true,
                },
        })),
    ...
    case "activity-layers": {
      const updates = highVolumeEvents(1, 8).map((event, index) => ({
        ...event,
        logicalEventId: `activity-layers-logical-${index + 1}`,
      }));
      const source = updates[0];
      if (!source)
        throw new Error("Activity layer scenario requires a source update.");
      const client = {
        ...(source.client ?? {}),
        id: source.client?.id ?? "activity-layers-client",
      };
      const clientStatus: LightstreamerEventEnvelope = {
        ...source,
        id: "activity-layers-client-status",
        kind: "client-status",
      ...
      const lostUpdates: LightstreamerEventEnvelope = {
        ...source,
        id: "activity-layers-lost-updates",
        kind: "lost-updates",
        subscription: undefined,
        listener: undefined,
        item: undefined,
      ...
      const subscriptionError: LightstreamerEventEnvelope = {
        ...source,
        id: "activity-layers-subscription-error",
        kind: "subscription-error",
        subscription: {
          ...source.subscription,
          id: "activity-layers-subscription",
        },
        listener: undefined,
        item: undefined,
        update: undefined,
        raw: { code: 500, message: "deterministic Activity layer error" },
      };
      return {
        id,
        initialEvents: [
    ...
    case "activity-clock-discontinuity": {
      ...
      const events = sourceEvents.map((event, index) => ({
        ...
        timestamp:
          anchor + (index < 4 ? index * 1_000 : -100_000 + (index - 4) * 1_000),
      }));
      return {
        id,
    ...
    case "activity-aggregation-failure":
      return {
        ...
        captureStatus: "capturing",
        activityProjectionFailure:
          "Synthetic Activity aggregation failure for browser verification.",
      };
    ...
    case "activity-rebucket": {
      const initial = highVolumeEvents(1, 8).map((event, index) => ({
        ...event,
        logicalEventId: `activity-rebucket-logical-${index + 1}`,
      }));
      const source = initial.at(-1);
      if (!source)
        throw new Error(
          "Activity rebucketing scenario requires a source update.",
        );
      return {
        id,
        ...
        deferredEvents: [
          {
            ...source,
            id: highVolumeEventId(9),
            logicalEventId: "activity-rebucket-logical-9",
            timestamp: source.timestamp + 200_000,
          },
        ],
        captureStatus: "capturing",
      };
    ...
    case "activity-ranking-pages":
      return {
        ...
        initialEvents: highVolumeEvents(1, 120).map((event, index) => ({
          ...
          subscription: {
            ...event.subscription,
            id: `activity-ranking-page-subscription-${index}`,
          },
          listener: {
            ...(event.listener ?? {
              id: `activity-ranking-page-listener-${index % 4}`,
            }),
            metricOwner: true,
          },
        })),
    ...
    case "filter-active-zero": {
      ...
      if (!statusEvent)
        throw new Error("The canonical scenario must include a Filter event.");
      return {
        id,
        initialEvents: [
          ...canonical,
    ...
    case "filter-collision": {
      ...
      if (!first || !second)
        throw new Error(
          "The canonical scenario must include two Filter events.",
        );
      return {
        id,
        initialEvents: [
          {
            ...first,
            id: "filter-collision-position",
            client: {
              ...(first.client ?? {}),
              id: first.client?.id ?? "collision-client",
              sessionId: "collision-session",
            },
            item: { name: "1", position: 1 },
          },
          {
            ...second,
            id: "filter-collision-name",
            client: {
              ...(second.client ?? {}),
              id: second.client?.id ?? "collision-client",
              sessionId: "collision-session",
            },
            item: { name: "1", position: 2 },
          },
        ],
        captureStatus: "capturing",
      };
    ...
    case "storage-headroom-warning":
      return {
        ...
        captureStatus: "capturing",
        storageEstimate: {
          usageBytes: 32 * 1_048_576,
          quotaBytes: 200 * 1_048_576,
        },
      };
    case "empty-scope":
    ...
    case "filter-find":
      return {
        ...
        selectedEventId: "scenario-event-3",
        captureStatus: "capturing",
        filterQuery: "scenario-event",
        findQuery: "item update",
      };
    case "filter-hidden-selection": {
      const passive = canonical[0];
      const selected = canonical.find(
        (event) => event.id === "scenario-event-3",
      );
      if (!passive || !selected?.update) {
        throw new Error(
          "The canonical scenario must include the passive and selected Item Updates.",
        );
      }
      const retainedDetails = JSON.stringify({
        passenger: { selected: false, priority: true },
      });
      const enrichedSelected: LightstreamerEventEnvelope = {
        ...
        update: {
          ...
          jsonPatches: {
            retainedDetails: [
              { op: "replace", path: "/passenger/selected", value: false },
            ],
          },
        },
      };
      return {
        id,
        initialEvents: canonical.map((event) =>
          event.id === selected.id ? enrichedSelected : event,
        ),
        laterEvents: [
          {
            ...passive,
            id: "scenario-event-1-passive",
            timestamp: passive.timestamp + 1000,
          },
        ],
        selectedEventId: "scenario-event-3",
        captureStatus: "capturing",
    ...
    case "command-projection-retention-failure": {
      const priorLocalSource = topology.capturedEvents.find(
        (event) => event.kind === "item-update",
      );
    ...
    case "local-injection-large": {
      ...
      if (!source?.update || !source.subscription)
        throw new Error(
          "Topology scenario requires a captured Item Update source.",
        );
    ...
    case "local-injection-scenario-review":
      return localInjectionScenario(id, true, 0, "delivered", {
        delayMs: 60_000,
        speed: 2,
        play: true,
      });
    ...
    case "local-injection-scenario-listener-drift":
      return localInjectionScenario(id, true, 0, "delivered", {
        driftEventKind: "listener",
      });
    case "local-injection-scenario-server-drift":
      return localInjectionScenario(id, true, 0, "delivered", {
        driftEventKind: "server",
      });
    ...
    case "local-injection-scenario-unretained":
      return {
        ...localInjectionScenario(id, true, 1, "delivered"),
        failLocalEvidenceRetention: true,
      };
    case "local-injection-scenario-cleared":
      return localInjectionScenario(id, true, 2, "delivered", {
        clearAfterRun: true,
      });
...
type DiagnosticCheckpointVisualState =
  "authoring" | "review" | "waiting" | "pass" | "fail" | "unavailable";

function localInjectionDiagnosticScenario(
  id: WorkbenchScenarioId,
  state: DiagnosticCheckpointVisualState,
): WorkbenchScenario {
  const affected = {
    kind: "subscription",
    pageId: "topology-small-page",
    clientId: "topology-small-client",
    sessionId: "topology-small-session",
    subscriptionId: "topology-small-subscription",
  } as const;
  const assertion: ScenarioAssertion = {
    id: "assertion-diagnostic",
    kind: "diagnostic-observation-exists",
    contractVersion: 1,
    ruleCode: "subscription.lost-updates",
  ...
  const diagnostic: DiagnosticObservationInput = {
    code: "subscription.lost-updates",
    severity: "error",
    lifecycle: { kind: "occurrence", occurrenceId: "scenario-lost-updates" },
    affected,
    observedAt: 1_780_872_000_900,
    observed: "SubscriptionListener reported lost updates.",
    limitation: "The callback reports a count but not the missing values.",
  ...
  return {
    ...
    ...(state === "pass"
      ? { diagnosticObservationsAfterReview: [diagnostic] }
      : {}),
    localInjection: {
      ...base.localInjection!,
      scenario: {
        ...
        checkpoints: [
          {
            name: diagnosticCheckpointVisualName(state),
...
function diagnosticCheckpointVisualName(
  state: DiagnosticCheckpointVisualState,
): string {
  switch (state) {
    case "authoring":
      return "Author normalized diagnostic observation";
    case "review":
      return "Reviewed diagnostic observation contract";
    case "waiting":
      return "Wait for a later lost-updates observation";
    case "pass":
      return "Later lost-updates observation exists";
    case "fail":
      return "Required lost-updates observation is absent";
    case "unavailable":
      return "Diagnostic Observation journal unavailable";
  }
}

type CheckpointVisualState =
  | "authoring"
  | "review"
  | "waiting"
  | "pass"
...
function localInjectionCheckpointScenario(
  id: WorkbenchScenarioId,
  state: CheckpointVisualState,
): WorkbenchScenario {
  const beforeSteps = state === "ambiguous-null";
  const assertions: readonly ScenarioAssertion[] =
    state === "waiting"
      ? [
          {
            id: "assertion-waiting-key",
            kind: "command-key-exists",
            item: { name: "topology-small-item", position: 1 },
            key: "never-arrives",
            expected: "present",
            withinActiveMs: 60_000,
          },
        ]
      : state === "pass"
        ? [
            {
              id: "assertion-correlated-evidence",
        ...
        : state === "fail"
          ? [
              {
                id: "assertion-mismatched-value",
                kind: "command-field-equals",
                item: { name: "topology-small-item", position: 1 },
                key: "small-alpha",
                field: "value",
                expected: "expected-but-not-observed",
              },
            ]
          : state === "wire-unavailable"
            ? [
                {
                  id: "assertion-wire-listeners",
                  kind: "listener-count",
                  stepId: "step-1",
                  count: "delivered",
                  expected: 1,
            ...
            : state === "ambiguous-null"
              ...
              : [
                  {
                    id: "assertion-delivered",
                    kind: "prior-injection-outcome",
                    stepId: "step-2",
                    expectedDisposition: "delivered",
                  },
                ];
  const review = state !== "authoring";
  const steps =
    state === "waiting" || state === "pass" || state === "fail"
      ? 3
      : state === "ambiguous-null"
        ? 1
        : 0;
  ...
  const configured: WorkbenchScenario = {
    ...
    localInjection: {
      ...
      scenario: {
        ...
        checkpoints: [
          {
            name: checkpointVisualName(state),
            assertions,
            ...(beforeSteps ? { beforeSteps: true } : {}),
          },
        ],
        review,
        steps,
...
function checkpointVisualName(state: CheckpointVisualState): string {
  switch (state) {
    case "authoring":
      return "Confirm the reviewed delivery";
    case "review":
      return "Reviewed local delivery contract";
    case "waiting":
      return "Wait for the committed key";
    case "pass":
      return "Correlated Local Evidence retained";
    case "fail":
      return "Expected failure did not occur";
    case "wire-unavailable":
      return "Listener count unavailable on wire";
    case "ambiguous-null":
      return "Server null requires attribution";
  }
}

function localInjectionCheckpointHighVolumeScenario(
  id: WorkbenchScenarioId,
): WorkbenchScenario {
    ...
    assertions: [
      {
        ...
        kind: "prior-injection-outcome" as const,
        stepId: `step-${index + 1}`,
        expectedDisposition: "delivered" as const,
      },
    ],
  }));
  return {
...
function withWireScenarioSource(
  scenario: WorkbenchScenario,
): WorkbenchScenario {
  const selected = scenario.initialEvents.find(
    ({ id }) => id === scenario.selectedEventId,
  );
  if (!selected?.update)
    throw new Error(
      "Wire Checkpoint scenario requires its selected Item Update.",
    );
  const wire = {
    ...selected,
    id: "scenario-wire-update",
    captureSource: "wire" as const,
    listener: undefined,
  };
  return {
    ...scenario,
    ...
    localInjection: {
      ...
      scenario: {
        ...scenario.localInjection!.scenario!,
...
function withAmbiguousServerNull(
  scenario: WorkbenchScenario,
): WorkbenchScenario {
  const selectedId = scenario.selectedEventId;
  const schemaEvents = scenario.initialEvents.map(
    (event): LightstreamerEventEnvelope =>
      event.kind === "item-update" && event.subscription
        ? {
            ...event,
            subscription: {
              ...event.subscription,
              fields: [
                ...(event.subscription.fields ?? ["command", "key", "value"]),
                "note",
              ],
            },
          }
  ...
  if (!selected?.update)
    throw new Error(
      "Ambiguous-null Checkpoint scenario requires its selected Item Update.",
    );
  ...
  const topologySyncFrames = scenario.topologySyncFrames?.map((frame) =>
    frame.type === "lsew:topology-sync-chunk"
      ? {
          ...frame,
          records: frame.records.map((record) =>
            record.kind === "subscription" && record.values?.subscription
              ? {
                  ...record,
                  values: {
                    ...record.values,
                    subscription: {
                      ...(record.values.subscription as Record<
                        string,
                        unknown
                      >),
                      fields: ["command", "key", "value", "note"],
                    },
                  },
                }
              : record,
      ...
      rawText: JSON.stringify(
        {
          ...
          fields: {
            ...
            key: "small-alpha",
            value: "1",
            note: "explicit Local replacement",
          },
        },
        null,
        2,
...
function localInjectionHighVolumeScenario(
  id: WorkbenchScenarioId,
): WorkbenchScenario {
  const base = localInjectionCapturedScenario(id);
  const representativeFields = Object.fromEntries(
    Array.from({ length: 500 }, (_, index) => [
      `field_${String(index + 1).padStart(3, "0")}`,
      `value-${index + 1}`,
    ]),
  ...
  return {
    ...base,
    topologySyncFrames: base.topologySyncFrames?.map((frame) =>
      frame.type === "lsew:topology-sync-chunk"
        ? {
            ...frame,
            records: frame.records.map((record) =>
    ...
    captureMessages: base.captureMessages?.map((message) =>
      message.payload.subscription
        ? {
            ...
            payload: {
              ...message.payload,
              subscription: {
                ...(message.payload.subscription as Record<string, unknown>),
                fields: schemaFields,
              },
            },
          }
    ...
    localInjection: {
      entry: "selection",
      rawText: JSON.stringify(
        { command: "ADD", key: "high-volume", isSnapshot: false, fields },
        null,
        2,
      ),
...
function localInjectionScenario(
  ...
  steps: number,
  executorOutcome:
    "delivered" | "delayed" | "partial" | "unknown" = "delivered",
  controls: Readonly<{
    delayMs?: number;
    speed?: 0.25 | 0.5 | 1 | 2 | 4;
    play?: boolean;
    driftEventKind?: "listener" | "server";
    clearAfterRun?: boolean;
  }> = {},
): WorkbenchScenario {
  const topology = getPanelScenario("topology-small");
  const source =
    topology.capturedEvents.find(({ id: eventId }) => eventId === "event-5") ??
    topology.capturedEvents.at(-1);
  if (!source?.update)
    throw new Error("Topology scenario requires a captured Item Update.");
  ...
  const driftEvent: LightstreamerEventEnvelope | undefined =
    controls.driftEventKind === "listener"
      ? undefined
      : controls.driftEventKind === "server"
        ? {
            ...second,
            id: "scenario-server-interleave",
            timestamp: source.timestamp + 2,
          }
        : undefined;
  const driftFrames =
    controls.driftEventKind === "listener"
      ? topology.topologySyncFrames?.map((frame) => {
          const common = {
            ...frame,
            syncId: "topology-small-listener-drift-sync",
            cutoffCaptureSequence: 7,
            recordCount: 7,
          };
          return frame.type === "lsew:topology-sync-chunk"
            ? {
                ...
                records: [
                  ...frame.records,
                  {
                    kind: "listener-attachment" as const,
                    id: "topology-small-listener-2-attachment",
                    parentId: "topology-small-subscription",
                    subscriptionId: "topology-small-subscription",
                    ...
                    values: {
                      clientId: "topology-small-client",
                      sessionId: "topology-small-session",
                      listenerId: "scenario-listener-2",
  ...
  return {
    ...
    localInjection: {
      ...
      terminalLimit: false,
      scenario: {
        addEventId: second.id,
        review,
        steps,
        ...(controls.delayMs !== undefined
          ? { delayMs: controls.delayMs }
          : {}),
        ...(controls.speed !== undefined ? { speed: controls.speed } : {}),
        ...(controls.play !== undefined ? { play: controls.play } : {}),
        ...(driftEvent ? { driftEvent } : {}),
        ...(driftFrames ? { driftFrames } : {}),
        ...(controls.clearAfterRun ? { clearAfterRun: true } : {}),
      },
    },
  };
}

function localInjectionCapturedScenario(
  id: WorkbenchScenarioId,
): WorkbenchScenario {
  const topology = getPanelScenario("topology-small");
  return {
...
function localInjectionJsonScenario(
  id: WorkbenchScenarioId,
): WorkbenchScenario {
  const topology = getPanelScenario("topology-small");
  const source = topology.capturedEvents.at(-1);
...
function diagnosticServerEvents(
  canonical: readonly LightstreamerEventEnvelope[],
): readonly LightstreamerEventEnvelope[] {
  const source = canonical[0];
  if (!source)
    throw new Error(
      "The canonical scenario must include a source event for diagnostics.",
    );
  const client = {
  ...
  const topology = (
    kind: "server-error" | "server-keepalive",
    captureSequence: number,
  ) => ({
    version: TOPOLOGY_OBSERVATION_VERSION,
    kind,
    ...
    client: {
      id: client.id,
      sessionId: { state: "real" as const, value: client.sessionId },
    },
  });
  const serverError: LightstreamerEventEnvelope = {
    ...
    subscription: undefined,
    listener: {
      id: "diagnostic-listener",
      callbacks: ["onServerError", "onServerKeepalive"],
    },
    item: undefined,
    update: undefined,
    serverError: {
      code: -7,
      message: "Application denied the operation",
      messageState: "safe",
    },
    keepalive: undefined,
  ...
  const serverKeepalive: LightstreamerEventEnvelope = {
    ...
    keepalive: {
      count: 14,
      windowId: "diagnostic-session:1",
      firstObservedAt: source.timestamp,
      lastObservedAt: source.timestamp + 2,
      aggregate: true,
    },
...
function diagnosticSubscriptionContextEvents(
  canonical: readonly LightstreamerEventEnvelope[],
): readonly LightstreamerEventEnvelope[] {
  const source = canonical[0];
  if (!source)
  ...
  const topology = (
    ...
    provenance: { instrumentationSource: "official-public-api" as const },
    coverage: {
      status: reason ? ("partial" as const) : ("complete" as const),
      ...(reason ? { reason } : {}),
      getters: {},
    },
  });
  ...
  const events: LightstreamerEventEnvelope[] = [
    ...
    configured("semantic-overlap", "MERGE", 4, 10),
    configured("raw-capability", "RAW", 5, 20),
  ];
  for (let index = 0; index < 6; index += 1) {
    const kind =
      index % 2 === 0
        ? ("listener-added" as const)
        : ("listener-removed" as const);
    events.push({
      ...source,
      ...
      update: undefined,
      raw: {
        callback: kind === "listener-added" ? "addListener" : "removeListener",
      },
...
function diagnosticAnomalyEvents(
  canonical: readonly LightstreamerEventEnvelope[],
): readonly LightstreamerEventEnvelope[] {
  const source = canonical[0];
  if (!source)
  ...
  const updates = canonical
    .slice(0, 4)
    .map((event, index): LightstreamerEventEnvelope => ({
      ...event,
...
/**
 * Browser-only Evidence fixture for the shared Activity track. The retained
 * records all belong to one client Session; duplicate listener deliveries use
 * one logical id, so the visible totals must not confuse deliveries with
 * Logical Updates. Timestamps are deliberately fixed rather than generated at
 * test time so a selected Activity range always maps to the same Filter.
 */
  ...
  const localUpdates = Array.from(
    ...
    (_, index): LightstreamerEventEnvelope => {
      ...
      return {
        ...
        timestamp: startedAt + 90_000 + index * 1_000,
        direction: "inbound",
        source: "synthetic",
        captureSource: "listener",
        synthetic: true,
...
function highVolumeEvents(
  ...
  if (!source) {
    throw new Error(
      "The canonical COMMAND scenario must include an UPDATE item for high-volume Evidence.",
    );
  }
  return Array.from(
    { length: count },
    (_, offset): LightstreamerEventEnvelope => {
      const sequence = first + offset;
      const findAnchor =
        sequence === 5 || sequence === 2_050 || sequence === 3_995;
      const event: LightstreamerEventEnvelope = {
        ...source,
        ...
        client: {
          ...
          sessionId:
            "session-2026-08-05-primary-production-orders-command-stream",
        },
        subscription: {
          ...source.subscription,
        ...
        update: {
          ...
          fields: {
            ...
            ...(findAnchor
              ? { complete_find_anchor: "complete-retained-find-anchor" }
              : {}),
          },
          changedFields: {
            quantity_for_primary_execution_venue: String(sequence),
        ...
        raw: {
          ...
          sequence,
          ...(findAnchor
            ? { findAnchor: "complete-retained-find-anchor" }
            : {}),
        },
      };
      if (sequence === 3_969 && event.update) {
        return {
          ...
          update: {
            ...
            fields: {
              ...event.update.fields,
              key: "field-only-key-must-not-be-inferred",
            },
          },
        };
      }
...
function highScopeEvents(
  ...
  if (!source) {
    throw new Error(
      "The canonical COMMAND scenario must include an UPDATE item for high Scope Evidence.",
    );
  }
  return Array.from({ length: count }, (_, offset) => {
    ...
    return {
      ...
      client: {
        ...source.client,
        id: "high-scope-client",
        sessionId: "high-scope-session",
      },
      subscription: {
        ...source.subscription,
      ...
      item: {
        ...source.item,
        name: `high-scope-item-${String(sequence).padStart(3, "0")}`,
        position: 1,
      },
      update: {
        ...source.update,
