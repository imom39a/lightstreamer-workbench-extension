...
export function ObservedActivityDocument({ runtime, activity, scopeLabel }: { runtime: WorkbenchRuntime; activity: WorkbenchActivitySnapshot; scopeLabel: string }): ReactElement {
  ...
  const selectionText = selectedBucket
    ...
    : selectedMarker
      ? `${selectedMarker.timestamp} · ${selectedMarker.label} · ${selectedMarker.reportedCount === null ? "reported count unavailable" : `reported ${selectedMarker.reportedCount}`} · Evidence ${selectedMarker.eventId}`
      : selectedRanking
  ...
  return <main className="workbench-react__document workbench-react__activity" aria-label="Observed Activity">
    ...
    <div className="workbench-react__activity-scroll">
      ...
      <section className="workbench-react__activity-section" aria-label="Activity selection detail"><h2>Selection</h2>{selectionText ? <><p>{selectionText}</p>{selectedBucket ? <button type="button" onClick={() => runtime.dispatch({ type: "show-activity-supporting-evidence", start: selectedBucket.start, end: selectedBucket.end })}>Show supporting Evidence</button> : <p>Show supporting Evidence is available for time selections.</p>}</> : <p>Select a bucket, marker, or ranked identity to inspect exact supporting facts.</p>}</section>
