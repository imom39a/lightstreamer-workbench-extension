...
export function LocalInjectionScenarioDocument({ runtime, snapshot }: Props): JSX.Element | null {
  ...
  return <section className="workbench-react__scenario" aria-label="Local Injection Scenario" data-phase={state.phase} onFocusCapture={(event) => {
    ...
    <footer className="workbench-react__local-footer">
      ...
      {state.phase === "paused" ? <><button type="button" onClick={() => dispatchWithFocus("edit", { type: "edit-scenario" })}>Edit Scenario</button><span>{state.runner?.pauseReason === "HIDDEN" ? "PAUSED — PANEL HIDDEN · explicit Resume required" : state.runner?.pauseReason === "DRIFT" ? "PAUSED — DRIFT · re-review required" : state.runner?.controlCapacityReached ? "PAUSED — CONTROL TRACE CAPACITY REACHED · Edit and Review again to continue." : `PAUSED · ${state.runner?.remainingDelayMs ?? 0} ms remains before Step ${next?.ordinal ?? "—"}`}</span><button type="button" disabled={!next || state.runner?.pauseReason === "DRIFT" || state.runner?.controlCapacityReached} onClick={() => dispatchWithFocus("resume-or-run-again", { type: "step-next-scenario" })}>Step next</button><button type="button" data-scenario-control="resume" className="workbench-react__primary" disabled={!next || !snapshot.visible || state.runner?.pauseReason === "DRIFT" || state.runner?.controlCapacityReached} onClick={() => dispatchWithFocus("pause", { type: "play-scenario" })}>Resume</button><button type="button" data-scenario-control="stop" onClick={() => dispatchWithFocus("stop-or-run-again", { type: "stop-scenario" })}>Stop</button></> : null}
