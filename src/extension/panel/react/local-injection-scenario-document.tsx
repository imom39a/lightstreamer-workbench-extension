...
export function LocalInjectionScenarioDocument({ runtime, snapshot }: Props): JSX.Element | null {
  ...
  useLayoutEffect(() => {
    ...
    if (!target) return;
    target.focus();
    if (intent !== "stop-or-run-again" || target.dataset.scenarioControl === "run-again") focusIntent.current = null;
  }, [state?.phase, state?.runner?.phase, state?.runner?.pauseReason]);
  ...
  return <section className="workbench-react__scenario" aria-label="Local Injection Scenario" data-phase={state.phase}>
    ...
    <footer className="workbench-react__local-footer">
      ...
      {state.phase === "paused" ? <><button type="button" onClick={() => dispatchWithFocus("edit", { type: "edit-scenario" })}>Edit Scenario</button><span>{state.runner?.pauseReason === "HIDDEN" ? "PAUSED — PANEL HIDDEN · explicit Resume required" : state.runner?.pauseReason === "DRIFT" ? "PAUSED — DRIFT · re-review required" : `PAUSED · ${state.runner?.remainingDelayMs ?? 0} ms remains before Step ${next?.ordinal ?? "—"}`}</span><button type="button" disabled={!next || state.runner?.pauseReason === "DRIFT"} onClick={() => dispatchWithFocus("resume-or-run-again", { type: "step-next-scenario" })}>Step next</button><button type="button" data-scenario-control="resume" className="workbench-react__primary" disabled={!next || !snapshot.visible || state.runner?.pauseReason === "DRIFT"} onClick={() => dispatchWithFocus("pause", { type: "play-scenario" })}>Resume</button><button type="button" data-scenario-control="stop" onClick={() => dispatchWithFocus("stop-or-run-again", { type: "stop-scenario" })}>Stop</button></> : null}
