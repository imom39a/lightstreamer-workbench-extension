...
- Every Scenario Step keeps its own stable identity, immutable Injection Source when present, raw-JSON Injection Draft, validation, comparison, undo state, and relative delay. Scenario Assertions belong to explicit zero-Injection Checkpoints. Duplicating or removing a Step is explicit; removal is undoable while editing. Captured Evidence sequence supplies the initial order for a confirmed multi-add, while execution order changes only through deliberate reordering.
- Captured Source fields carry replayability evidence into the Step. Sanitized/redacted values, unavailable fields, and unresolved wire differences never become executable values merely because they occupy a JSON slot. Scenario Review remains blocked until each such field is explicitly replaced with a concrete Draft value or removed where the target schema permits it; Workbench never injects a redaction marker as though it were captured application data.
...
Scenario Review validates every Step, Source-field replayability, the shared Target, capacity, assertions, and timing, then freezes the exact Scenario revision, Step order, payloads, relative delays, speed, assertion windows, target identity, target fingerprint, and committed-Evidence seed boundary into a Scenario Run. Capture and normalization must distinguish a sanitized value from an identical literal application string and preserve unresolved wire-difference limitations so this check can fail closed. Editing returns to a new Scenario revision and cannot mutate an existing Run.
...
- The validate → fingerprint → execute once → await Evidence settlement → map outcome path is one renderer-neutral Local Injection execution coordinator shared by standalone Drafts and Scenario Steps.
...
- Build 4 selected-update inspection and the full Build 5 diagnostics/linting experience are independent enhancements, not prerequisites for the core Scenario workflow. Scenario and Build 4 share only the narrow headless Item Update replayability/value-semantics contract described above. The additive diagnostic-presence assertion family alone depends on Build 5's normalized diagnostic observation contract.
