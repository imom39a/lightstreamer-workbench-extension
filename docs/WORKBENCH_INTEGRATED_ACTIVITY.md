...
One collapsed-by-default **Activity summary** belongs in existing Context, labelled for its Scope and available alongside selected Evidence. It retains exact SERVER and LOCAL Logical Update and Update Delivery counts, the SERVER Snapshot/Live breakdown, and one bounded SERVER-only busiest Subscriptions/items list. Captured bandwidth and frequency facts remain contextual. An Activity aggregation failure is an active Workbench condition in the global footer and Notifications; its footer copy may be dismissed without hiding the retained notification or changing Evidence. Other active operational conditions follow the same lifecycle, while captured diagnostic markers link to Evidence rather than duplicating banners.
