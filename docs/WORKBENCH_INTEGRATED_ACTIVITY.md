...
The ruler shows elapsed time **since the first retained event** in the current History Interval. This is not a claim about when Capture or the Session began. The origin remains stable through Scope, Filter and selected-range changes; Clear establishes a new origin. Timeline bounds, counts and marks use the complete retained History at the current committed or Frozen read point. Clock regressions and unavailable evidence remain explicit limits rather than invented durations or zeros.
