...
- [Local Injection interaction model](../prototypes/workbench-ui-05/COMPARISON.md) — one protected standalone target-anchored raw-JSON Injection Draft and the separate accepted Scenario boundary.
...
- Keep active operational Capture, History, storage, Activity-aggregation, canonical current-runtime recovery, and runtime-Scope conditions in the global footer until they end or the developer individually dismisses their footer copy. Every dismissible footer condition also belongs in the promoted **Notifications** document; dismissal changes presentation only, never Evidence or its Diagnostic Observation, and resolution clears the dismissal so a later recurrence surfaces again. Recurring Lightstreamer notices belong once in Notifications, with supporting Evidence or affected Scope routes, not in Ordered Evidence or Context banners. Notification filters are independent of Evidence Scope and Filter. At shallow geometry, preserve a complete condition heading, affected identity, Dismiss action, and visible details cue instead of clipping long copy. Keep workflow-local validation and outcomes at their own decision boundary.
...
- The standalone workflow reviews and injects one focused Draft. There is no direct Injection or Scenario execution keyboard shortcut.
